...
class Users(db.Model):
    ...
    type = db.Column(db.String(3), nullable=False, default='helpee') # can bee 'helpee' or 'helper'
...
class UserPermissions(db.Model):
    ...
    accepted_terms = db.Column(db.Boolean, nullable=False, defult=False) 
    accepted_gdpr = db.Column(db.Boolean, nullable=False, defult=False)
    accepted_health_safety = db.Column(db.Boolean, nullable=False, defult=False)
...
class Jobs(db.Model):
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    helper_id = db.Column(db.Integer, db.ForeginKey('users.id'), nullable=False)
    requester_id = db.Column(db.Integer, db.ForeginKey('users.id'), nullable=False)
